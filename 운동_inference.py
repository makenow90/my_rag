...
def process_document_for_book(query, book_name, query_embedding, embedding_model,first_id):
    ...
    retriever = vectorstore.as_retriever(
        ...
        search_kwargs={'k': 100, "score_threshold": 0.35}
    )
...
def convert_common_to_medical(query):
    ...
    for common_term, medical_term in common_to_medical_terms.items():
...
def 운동_inference(query, book_names):
    ...
    # 13. Ollama 모델 설정
    # model = ChatOllama(model="llama3.1:70b", temperature=0.5)
